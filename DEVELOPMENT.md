...
> ⚠️ **版本号规则（2026-08-01，开发端必须遵守）**：版本更新只能由「测试/架构（`C:\Users\86133\Desktop\测试gym\GymFlow改进报告.md` §17）」与「用户」共同推动。**开发端禁止自行改动 `APP_VERSION`（js/data.js）、`CACHE_NAME`（sw.js）、CHANGELOG.md、git 版本号/tag**。你只负责实现功能与修 bug；版本指派、大版本（1→2）确认、发版时机由用户+测试端决定。若发现测试端指派的版本与代码不一致，上报用户，不要自行对齐。
